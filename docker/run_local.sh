...
# run
mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=8080 -Dspring-boot.run.profiles=local &

# check if GPD is UP and open api spec
URL_CHECK="http://localhost:8080/v3/api-docs"
...
# open -na "Google Chrome" --args -incognito http://localhost:8080/swagger-ui/index.html
