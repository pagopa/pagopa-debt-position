...
public class ErrorHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ProblemJson> handleDataIntegrityViolationException(final DataIntegrityViolationException ex,
			final WebRequest request) {
		ProblemJson errorResponse = null;

		if (ex.getCause() instanceof ConstraintViolationException) {
			log.warn("Constraint violation from Database", ex);
			errorResponse = ProblemJson.builder().status(HttpStatus.CONFLICT.value())
					.title("Conflict with the current state of the resource")
					.detail("There is a relation with other resource.").build();
		...
		if (errorResponse == null) {
			...
			errorResponse = ProblemJson.builder().status(HttpStatus.INTERNAL_SERVER_ERROR.value())
