...
class ValidTransferListValidatorTest {
  ...
  private final ValidTransferListValidator validTransferListValidator =
      new ValidTransferListValidator();
  ...
  void shouldReturnFalseWhenTransferHaveWrongSize(List<String> transferModelIdsList) {
    ...
    verify(context)
        .buildConstraintViolationWithTemplate(
            "Transfer list must contain between 1 and 5 transfers");
  }

  public static Stream<Arguments> invalidTransferModelIdsTestMethodSource() {
    return Stream.of(Arguments.of("0"), Arguments.of("invalid"), Arguments.of("6"));
  }
  ...
  void shouldReturnFalseWhenTransferListHaveInvalidIds(String invalidId) {
    ...
    verify(context)
        .buildConstraintViolationWithTemplate(
            "Transfer list contains invalid transfer ids: [%s]".formatted(invalidId));
  }
  ...
  void shouldReturnFalseWhenTransferListHaveDuplicatedIds() {
    ...
    assertFalse(validTransferListValidator.isValid(transferModelList, context));
    verify(context)
        .buildConstraintViolationWithTemplate("Transfer list invalid value: [1], expected: [2]");
  }
  ...
  public static Stream<Arguments> invalidTransferListMissingIdsMethodSource() {
    return Stream.of(
        ...
        Arguments.of(
            List.of("1", "2", "3", "5"), "Transfer list invalid value: [5], expected: [4]"));
  }

  @ParameterizedTest
  @MethodSource("invalidTransferListMissingIdsMethodSource")
  void shouldReturnFalseWhenTransferListWithMissingId(
      List<String> transferModelIdsList, String expectedErrorMessage) {
    List<TransferModel> transferModelList =
        transferModelIdsList.stream()
            .map(
                id -> {
  ...
  public static Stream<Arguments> validTransferModelTestMethodSource() {
    ...
    IntStream.range(1, 6)
        .forEach(
            i ->
                validIdsSequences.add(
                    IntStream.range(1, i + 1).boxed().map(String::valueOf).toList()));

    return validIdsSequences.stream().map(Arguments::of);
  }
