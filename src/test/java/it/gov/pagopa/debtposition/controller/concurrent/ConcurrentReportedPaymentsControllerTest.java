...
class ConcurrentReportedPaymentsControllerTest {
  ...
  void initDebtPosition_for_reporting() throws Exception {
    ...
    mvc.perform(
            post("/organizations/REPORT_Reported_12345678901/debtpositions?toPublish=true")
    ...
    mvc.perform(
            post("/organizations/REPORT_Reported_12345678901/paymentoptions/"
                    + auxDigit
    ...
    mvc.perform(
            get("/organizations/REPORT_Reported_12345678901/debtpositions/12345678901IUPDMULTIPLEMOCK8")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void concurrent_reportedTransfer_id_1_thread(int number) throws Exception {
    log.trace(
        "concurrent_reportedTransfer_id_1_thread for transfers with id 1 - thread("
            + number
            + ") start => "
    ...
    // lo IUV e non il NAV)
    mvc.perform(
            post("/organizations/REPORT_Reported_12345678901/paymentoptions/123456IUVMULTIPLEMOCK8/transfers/1/report")
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void concurrent_reportedTransfer_id_2_thread(int number) throws Exception {
    log.trace(
        "concurrent_reportedTransfer_id_2_thread for transfers with id 2 - thread("
            + number
            + ") start => "
            + Thread.currentThread().getName());
    // effettuo la rendicontazione per una delle 2 transazioni della PO (si continua ad utilizzare
    // lo IUV e non il NAV)
    mvc.perform(
            post("/organizations/REPORT_Reported_12345678901/paymentoptions/123456IUVMULTIPLEMOCK8/transfers/2/report")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().is(oneOf(200, 409)));
    log.trace(
        "concurrent_reportedTransfer_id_2_thread for transfers with id 2 - thread("
  ...
  void checkDebtPosition_after_reporting() throws Exception {
    ...
    String url =
        "/organizations/REPORT_Reported_12345678901/paymentoptions/"
            + auxDigit
