...
class ConcurrentReportedPaymentsControllerTest {
  ...
  void initDebtPosition_for_reporting() throws Exception {
    ...
    mvc.perform(
            post("/organizations/12345678901999999/debtpositions?toPublish=true")
    ...
    mvc.perform(
            get("/organizations/12345678901999999/debtpositions/12345678901IUPDMULTIPLEMOCK8")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void concurrent_reportedTransfer_id_1_thread(int number) throws Exception {
    ...
    // lo IUV e non il NAV)
    mvc.perform(
            post("/organizations/12345678901999999/paymentoptions/1234568/transfers/1/report")
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void concurrent_reportedTransfer_id_2_thread(int number) throws Exception {
    ...
    // lo IUV e non il NAV)
    mvc.perform(
            post("/organizations/12345678901999999/paymentoptions/1234568/transfers/2/report")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().is(oneOf(200, 409)));
    log.trace(
        "concurrent_reportedTransfer_id_2_thread for transfers with id 2 - thread("
