...
class DebtPositionControllerV3Test {
  ...
  void createDebtPosition_400_duplicateInstallmentMetadataKey() throws Exception {
    ...
    mvc.perform(post(uri).content(TestUtil.toJson(pp)).contentType(MediaType.APPLICATION_JSON))
        ...
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(
            jsonPath("$.detail").value(containsString("installmentMetadata keys must be unique")));
  }
  ...
  void createDebtPosition_400_duplicateTransferMetadataKey() throws Exception {
    ...
    mvc.perform(post(uri).content(TestUtil.toJson(pp)).contentType(MediaType.APPLICATION_JSON))
        ...
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(
            jsonPath("$.detail").value(containsString("transferMetadata keys must be unique")));
  }
        ...
        .andExpect(
            jsonPath(
                ...
                CoreMatchers.containsString(
                    "paymentOption[0].installments[0].transfer: Transfer list must contain between 1 and 5 transfers")));
  }
    ...
    mvc.perform(
        ...
        .andExpect(
            jsonPath(
                "$.detail",
                CoreMatchers.containsString(
                    "paymentOption[0].installments[0].transfer: Transfer list contains invalid transfer ids: [%s]"
                        .formatted(invalidId))));
  }
    ...
    mvc.perform(
        ...
        .andExpect(
            jsonPath(
                "$.detail",
                CoreMatchers.containsString(
                    "paymentOption[0].installments[0].transfer: Transfer list invalid value: [1], expected: [2]")));
  }
    ...
    mvc.perform(
        ...
        .andExpect(
            jsonPath(
                "$.detail",
                CoreMatchers.containsString(
                    "paymentOption[0].installments[0].transfer: %s"
  ...
  void createAndGet_PaymentOptionDescription_and_InstallmentDescription() throws Exception {
    ...
    mvc.perform(get(getUri).contentType(MediaType.APPLICATION_JSON))
        ...
        .andExpect(
            jsonPath("$.paymentOption[1].installments[0].description").value("Piano A - Rata 1/1"));
  }
  ...
  public static PaymentPositionModelV3 createPaymentPositionV3(
      int numberOfPO, int numberOfInstallment, List<String> transferIds) {
    PaymentPositionModelV3 paymentPosition =
        createPaymentPositionV3(numberOfPO, numberOfInstallment);
    paymentPosition
        .getPaymentOption()
        .forEach(
