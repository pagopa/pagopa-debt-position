...
class PaymentsServiceTest {
  ...
  void updateTransferIbanMassive_OK() {
    ...
    doReturn(List.of(pp))
        ...
        .findByOrganizationFiscalCodeAndStatusIn(anyString(), any());

    PaymentOption po = PaymentOption.builder().build();
    doReturn(List.of(po))
        .when(paymentOptionRepository)
        .findByPaymentPositionInAndStatusIn(any(), any());

    doReturn(1)
        .when(transferRepository)
        .updateTransferIban(any(), anyString(), anyString(), any(LocalDateTime.class));
  ...
  void updateTransferIbanMassive_KO_noPaymentPosition() {
    ...
    assertThrows(
        AppException.class,
        () -> paymentsService.updateTransferIbanMassive("orgFiscalCode", "oldIban", "newIban"));
  }
