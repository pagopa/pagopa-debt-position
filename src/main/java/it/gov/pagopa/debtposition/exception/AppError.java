...
public enum AppError {
    ...
    DEBT_POSITION_IN_UPDATABLE_STATE_NOT_FOUND(HttpStatus.NOT_FOUND, "No debt position in updatable state found", "Not found a debt position in updatable state for Organization Fiscal Code %s"),
    ...
    PAYMENT_OPTION_NOTIFICATION_FEE_UPDATE_NOT_UPDATABLE(HttpStatus.UNPROCESSABLE_ENTITY, "Not in unpaid state for notification fee update", "The payment option with Organization Fiscal Code %s and NAV %s is not in unpaid state and the notification fee cannot be updated."),
    ...
    PAYMENT_OPTION_PAY_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "The pay call for a payment option is failed", "Payment failed for the payment option with Organization Fiscal Code %s and NAV %s"),
