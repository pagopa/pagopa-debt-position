...
public class PaymentOptionByAttribute implements Specification<PaymentOption> {
    ...
    public PaymentOptionByAttribute(PaymentPosition paymentPosition, LocalDateTime dateFrom, LocalDateTime dateTo, List<String> segregationCodes) {
    ...
    public Predicate toPredicate(Root<PaymentOption> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
    	...
    	Predicate paymentPositionIdPredicate = cb.equal(root.get("paymentPosition"), this.paymentPosition);
