...
public interface PaymentPositionRepository
   ...
   * records to prevent connection pool exhaustion. This should be called in a loop until fewer
   * records are affected than batchSize.
   */
  ...
  /**
   * Rule 6- For a debt position to EXPIRED, the following conditions must be met: 1. all POs must
   * be PO_UNPAID 2. all POs must have switch_to_expired = true 3. all POs must be expired
   *
   * <p>Batch version of updatePaymentPositionStatusToExpired for OPTIMIZATION: Updates maximum
   * batchSize records to prevent connection pool exhaustion. This should be called in a loop until
   * fewer records are affected than batchSize.
   */
  @Modifying(clearAutomatically = true, flushAutomatically = true)
  @Query(
      value =
