...
public interface IDebtPositionController {
  ...
  ResponseEntity<PaymentPositionsInfo> getOrganizationDebtPositions(
      ...
      @Valid
          @Parameter(
              description =
                  "Filter from payment_date_time (if provided use the format yyyy-MM-ddTHH:mm:ss)."
                      + " If not provided will be set to 30 days before the"
      ...
      @Valid
          @Parameter(
              description =
                  "Filter to payment_date_time (if provided use the format yyyy-MM-ddTHH:mm:ss). If"
                      + " not will be set to 30 days after the payment_date_time_to.")
          @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
