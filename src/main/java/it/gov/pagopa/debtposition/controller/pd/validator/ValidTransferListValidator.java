...
public class ValidTransferListValidator
  ...
  private static final Set<String> VALID_TRANSFER_ID_SET =
      IntStream.range(1, ValidTransferList.MAX_TRANSFER_LIST_SIZE + 1)
          .boxed()
          .map(String::valueOf)
  ...
  public boolean isValid(List<TransferModel> transferList, ConstraintValidatorContext context) {
    ...
    if (transferList == null
        ...
        || transferList.size() > ValidTransferList.MAX_TRANSFER_LIST_SIZE) {
      formatMessage(
          context,
          "Transfer list must contain between %s and %s transfers"
              .formatted(1, ValidTransferList.MAX_TRANSFER_LIST_SIZE));
      return false;
    }
    List<String> transferIds = transferList.stream().map(TransferModel::getIdTransfer).toList();
    List<String> invalidIds =
        transferIds.stream().filter(Predicate.not(VALID_TRANSFER_ID_SET::contains)).toList();
    ...
    for (String transferIdx : transferIds) {
      ...
      if (Integer.parseInt(transferIdx) != currentIdx) {
        formatMessage(
            context,
            "Transfer list invalid value: [%s], expected: [%d]".formatted(transferIdx, currentIdx));
        return false;
      }
      currentIdx++;
    }
