        ...
        try { // TODO VERIFY
            for (int i = 0; i < organizationPpList.size(); i++) {
                ...
                for (PaymentOption po : pp.getPaymentOption()) {
                    for (Installment installment : po.getInstallment()) {
                        // bulk operation to persist the Installment debt position info
                        if (numOfBatchOperations < maxBatchOperationSize - 1
                                && i < organizationPpList.size() - 1) {
                            TableEntity tableEntity = new TableEntity(pp.getOrganizationFiscalCode(), installment.getIuv());
                            Map<String, Object> properties = new HashMap<>();
                            properties.put("PaymentDate", installment.getPaymentDate());
