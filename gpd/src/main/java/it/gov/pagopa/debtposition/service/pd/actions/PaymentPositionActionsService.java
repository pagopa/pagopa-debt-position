...
public class PaymentPositionActionsService {
	...
	public PaymentPosition publish(@NotBlank String organizationFiscalCode, @NotBlank String iupd) {
		...
		// se non era stata prevista una data di inizio validità e la data di pubblicazione è < della min_due_date => sovrascrivo lo stato direttamente a VALID
		...
		// se invece la richiesta di pubblicazione è avvenuta dopo che una una delle opzioni di pagamento è scaduta (currentDate > min_due_date) viene rilanciato un errore
		...
		// se la richiesta di pubblicazione viene fatta dopo che è già passata la data di inizio validità viene rilanciato un errore
